...
monomial_term* parse_args(char *arg1, uint32_t *list_size){
	...
	while(*arg1 != '\0'){
		...
		arg1 += increment;
		term_list = realloc(term_list, sizeof(monomial_term) * term_list_length + 1);
...
void initialize_function_sample(monomial_term *term_list, uint32_t terms_length, function_sample *sample, char **argv) {
	argv++; argv++;
...
coordinate* sample_function_sequential(function_sample *sample){
	...
	printf("Total coordenadas a calcular: %u\n", total_coords);
	if(coords == NULL){
